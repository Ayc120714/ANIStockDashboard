...
const deriveTier = (row) => {
  const explicitTier = String(row.buy_sell_tier || '').trim().toUpperCase();
  if (explicitTier && tierColors[explicitTier]) return explicitTier;
  const signalType = String(row.signal_type || '').toLowerCase();
  if (signalType.includes('strong_buy')) return 'B3';
  if (signalType.includes('buy')) return 'B1';
  if (signalType.includes('strong_sell')) return 'S3';
  if (signalType.includes('sell')) return 'S1';
  const recommendation = String(row.recommendation || row.rating || '').toLowerCase().replace(/\s+/g, '_');
  if (recommendation === 'strong_buy') return 'B3';
  if (recommendation === 'buy') return 'B1';
  if (recommendation === 'strong_sell') return 'S3';
  if (recommendation === 'sell') return 'S1';
  const score = parseNumber(row.signal_score ?? row.composite_score);
  if (score == null) return null;
  if (score >= 70) return 'B3';
  if (score >= 45) return 'B2';
  if (score >= 25) return 'B1';
  if (score <= -70) return 'S3';
  if (score <= -45) return 'S2';
  if (score <= -25) return 'S1';
  return null;
};
...
function ShortTermPage() {
  ...
  useEffect(() => {
    ...
    const loadAlerts = async () => {
      try {
        const rows = await fetchPriceAlerts({ userId, listType: 'short_term', activeOnly: true });
        ...
        try {
          ...
          if (mounted) setPriceAlerts(Array.isArray(parsed) ? parsed : []);
        } catch {
  ...
  }, [priceAlertsKey, userId]);

  const existingSymbols = useMemo(
    () => new Set(data.map((d) => normalizeSymbol(d.symbol)).filter(Boolean)),
    [data]
  );
  ...
  const handleDeleteFromWatchlist = async (symbol) => {
    ...
    if (!sym) return;
    if (!window.confirm(`Remove ${sym} from Short Term watchlist?`)) return;
    setDeleting(true);
    try {
  ...
  const buildProductProfiles = useCallback((row) => {
    ...
    const baseT1 = parsePrice(row?.target_short_term ?? row?.target_1) || 0;
    ...
    return {
      ...
      byProduct: {
        ...
        DELIVERY: {
          ...
          target1: parsePrice(row?.target_long_term) || baseT2 || proj(risk * 3),
          target2: proj(risk * 4),
          deliveryLeverage: genericDeliveryLeverage,
        },
      },
  ...
  const handleSetAlert = useCallback(({ symbol }) => {
    ...
    const row = data.find((r) => normalizeSymbol(r.symbol) === cleanSymbol);
  ...
  useEffect(() => {
    if (!priceAlerts.length || !merged.length) return;
    const prices = merged.reduce((acc, row) => {
      const price = parsePrice(row?.price);
      if (price != null) acc[normalizeSymbol(row.symbol)] = price;
      return acc;
    }, {});
    checkPriceAlerts({ userId, listType: 'short_term', prices })
      .then((res) => {
  ...
  return (
    <TableSection>
      ...
      <OrderPanel
        ...
        symbolOptions={merged.map((r) => r.symbol)}
        symbolPrices={Object.fromEntries(merged.map((r) => [normalizeSymbol(r.symbol), parsePrice(r.price) || 0]))}
        symbolProfiles={Object.fromEntries(merged.map((r) => {
            ...
            <tbody>
              {paged.map(row => (
                ...
                >
                  ...
                  <td>
                    {deriveTier(row) ? (
                      <Chip label={deriveTier(row)} size="small"
                        sx={{ bgcolor: tierColors[deriveTier(row)] || '#666', color: '#fff', fontWeight: 700, fontSize: 12 }} />
                    ) : '—'}
                  </td>
