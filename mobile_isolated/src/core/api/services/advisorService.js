...
export const advisorService = {
  ...
  fetchMlSetups: ({min_score = 0.55, setup_type = '', side = '', limit = 500, timeoutMs} = {}) =>
    apiGet(
      `/advisor/signals/ml-setups${toQuery({
        ...
        side: side === '' || side == null ? undefined : side,
        limit,
      })}`,
      {timeoutMs: timeoutMs ?? T},
    ),
  ...
  fetchStageEntryTiming: ({
    ...
    apiGet(
      `/advisor/signals/stage-entry-timing${toQuery({
        ...
        require_weekly_confirm: require_mtf_confirm || require_weekly_confirm ? true : undefined,
        entry_timing: entry_timing || undefined,
        refresh: refresh ? true : undefined,
      })}`,
      {timeoutMs: timeoutMs ?? T},
    ),
  /** Compression → breakout → light retest → renewed participation (1d / 1w). */
  fetchCompressionBreakout: ({
    ...
    apiGet(
      `/advisor/signals/compression-breakout${toQuery({
        timeframe,
        limit,
        symbol_limit,
        min_market_cap_cr,
        ...
        include_compression: include_compression ? true : undefined,
        refresh: refresh ? true : undefined,
      })}`,
      {timeoutMs: timeoutMs ?? T},
    ),
