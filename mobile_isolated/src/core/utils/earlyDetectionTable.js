...
export function dedupeEarlyDetectionBySymbol(rows) {
  const bySymbol = new Map();
  for (const row of rows || []) {
    const symbol = String(row?.symbol || '').trim().toUpperCase();
    if (!symbol) continue;
    const prev = bySymbol.get(symbol);
    if (!prev || Number(row?.trigger_ts || 0) > Number(prev?.trigger_ts || 0)) {
...
export function buildEarlyDetectionTableModel(rawRows, { sqzFilter, sortCol, sortDir } = {}) {
  const normalized = dedupeEarlyDetectionBySymbol(normalizeEarlyDetectionRows(rawRows));
