...
const isNormalizedBrokerRow = row =>
  Boolean(row && String(row.symbol || '').trim() && Number.isFinite(Number(row.net_qty)));
...
export const normalizeBrokerRows = payload => {
  ...
  const normalized = rows
    .map(row => {
      ...
      const isDeliveryHolding =
        toNumber(row?.dpQty ?? row?.dp_qty ?? row?.dpQuantity ?? row?.totalQty ?? row?.total_qty ?? row?.totalQuantity) > 0
